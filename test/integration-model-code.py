class MultiStageSystemWrapper(mlflow.pyfunc.PythonModel):
    def __init__(self, llm_model_name, catalog_name, schema_name):
    ...
    def create_code_regulation_rag_chain(llm_model):
        ...
        model_name = "hugging_face_sentence_transformer_model"
        model_version = 1
        vs_endpoint_name = f"vs_endpoint_{catalog_name}_test"
        ...
        def retrieve(inputs):
            ...
            query_embeddings = loaded_model.predict(pd.DataFrame({"text": [query]}))
    ...
    @staticmethod
    def create_bim_revit_data_chain(catalog_name, schema_name, llm_model):
        ...
        class RoomRelationshipTool:
            def __init__(self, data_retrieval_tool, catalog_name, schema_name, volume_name, template_filename):
                ...
                self.intents = {
                    "count_rooms": ["how many rooms", "count rooms", "total rooms"],
                    "count_connections": ["how many connections", "count connections", "total connections"],
                    "list_rooms": ["list rooms", "show rooms", "what rooms"],
                    "check_rooms_exist": ["check rooms exist", "do these rooms exist", "are these rooms present"]
                }
            ...
            def analyze(self, query=None):
                ...
                room_graph = GraphFrame(vertices, edges)
            ...
            def perform_analysis(self, room_graph, query):
                ...
                if intent == "count_rooms":
                    return f"Total number of rooms: {room_graph.vertices.count()}"
                elif intent == "count_connections":
                    return f"Total number of connections: {room_graph.edges.count()}"
                elif intent == "list_rooms":
                    rooms = room_graph.vertices.select("name").collect()
                    room_list = ", ".join([row["name"] for row in rooms])
                    return f"List of rooms: {room_list}"
                elif intent == "check_rooms_exist":
                    room_names = self.extract_room_names(query)
                    existing_rooms = [row["name"] for row in room_graph.vertices.select("name").collect()]
                    
                    existing = []
                    non_existing = []
                    for room in room_names:
                        best_match = process.extractOne(room, existing_rooms, score_cutoff=80)
                        if best_match:
                            existing.append(best_match[0])
                        else:
                            non_existing.append(room)
        ...
        class BIMRevitDataChain:
            def __init__(self, catalog_name, schema_name, llm_model):
                self.catalog_name = catalog_name
                self.schema_name = schema_name
            ...
            def setup_tools(self):
                ...
                self.path_calculation_tool = RoomPathCalculationTool(
                    self.data_retrieval_tool, self.catalog_name, self.schema_name, "engineering-volume", "room-route-visualisation-min.html"
                )
                self.room_relationship_tool = RoomRelationshipTool(
                    self.data_retrieval_tool, self.catalog_name, self.schema_name, "engineering-volume", "room-relationship-visualisation-min.html"
                )
            ...
            def setup_agent(self, query=None):
                ...
                Final Answer:  the final answer to the question and include any relevant notes as the value and the key is `final_answer` and the Visualization JSON with the key `visualization_json` as the key
                ...
                - For simple queries, use the tool immediately without overthinking.
        ...
        bim_chain = BIMRevitDataChain(catalog_name, schema_name, llm_model)
    ...
    def load_context(self, context):
        ...
        self.bim_revit_data_chain = self.create_bim_revit_data_chain(self.catalog_name, self.schema_name, llm_model)
...
mlflow.pyfunc.pickle_custom_model = cloudpickle.dumps
...
sample_input = pd.DataFrame([{"query": "What is the path from FCC to Staircase?", "debug_mode": True}])

# Define the input and output schema
input_schema = Schema([
    ColSpec("string", "query"),
    ColSpec("boolean", "debug_mode")
])
output_schema = Schema([ColSpec("string")])  # Assuming the output is a list of JSON strings
...
# Start an MLflow run
with mlflow.start_run(run_name="multi_stage_system_run") as run:
    mlflow.log_param("llm_model_name", llm_model_name)
    mlflow.log_param("catalog_name", catalog_name)
    mlflow.log_param("schema_name", schema_name)

    wrapper = MultiStageSystemWrapper(llm_model_name, catalog_name, schema_name)

    mlflow.pyfunc.log_model(
        artifact_path="multi_intent_regulation_bim_compliance_model",
        python_model=wrapper,
        input_example=sample_input,
        registered_model_name="multi_intent_regulation_bim_compliance_model",
        conda_env=conda_env,
        signature=signature,
    )
